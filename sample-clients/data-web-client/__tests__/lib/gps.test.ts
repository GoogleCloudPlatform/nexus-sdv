...
describe('extractGpsPoints', () => {
  ...
  it('returns empty array when only longitude is present (no latitude)', () => {
    const detail = makeDetail(['dynamic:gps.longitude'], []);
    expect(extractGpsPoints(detail)).toEqual([]);
  });

  it('extracts GPS points when altitude column is absent (defaults to 0)', () => {
    const detail = makeDetail(
      ['dynamic:GPS_LATITUDE', 'dynamic:GPS_LONGITUDE'],
      [
        {
          timestamp: '2026-04-21T08:02:12Z',
          values: {
            'dynamic:GPS_LATITUDE': '"41.49489"',
            'dynamic:GPS_LONGITUDE': '"2.1121676"',
          },
        },
      ],
    );
    expect(extractGpsPoints(detail)).toEqual([
      { timestamp: '2026-04-21T08:02:12Z', lat: 41.49489, lng: 2.1121676, alt: 0 },
    ]);
  });

  it('parses JSON-double-encoded numeric strings (e.g. "\\"41.5\\"")', () => {
    const detail = makeDetail(
      ['dynamic:GPS_LATITUDE', 'dynamic:GPS_LONGITUDE', 'dynamic:GPS_ALTITUDE'],
      [
        {
          timestamp: '2026-04-21T08:02:12Z',
          values: {
            'dynamic:GPS_LATITUDE': '"41.5"',
            'dynamic:GPS_LONGITUDE': '"-0.1"',
            'dynamic:GPS_ALTITUDE': '"10"',
          },
        },
      ],
    );
    expect(extractGpsPoints(detail)).toEqual([
      { timestamp: '2026-04-21T08:02:12Z', lat: 41.5, lng: -0.1, alt: 10 },
    ]);
  });
  ...
  it('extracts valid GPS points using uppercase underscore qualifier names (nats-bigtable-connector style)', () => {
    const detail = makeDetail(
      ['dynamic:GPS_LATITUDE', 'dynamic:GPS_LONGITUDE', 'dynamic:GPS_ALTITUDE'],
      [
        {
  ...
  it('returns empty array when only some GPS underscore columns present', () => {
    const detail = makeDetail(['dynamic:GPS_LATITUDE', 'dynamic:GPS_LONGITUDE'], []);
    expect(extractGpsPoints(detail)).toEqual([]);
  });
